...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 判断当前线程是否存在事务、判断依据为当前线程记录的链接不为空、且链接中的 transactionActive 属性为true
	...
	/**
	 * 在我们分析事务异常处理规则的时候、当某个事务既没有保存点、也不是新事务、spring 对其
	 * 的处理只是设置一个回滚标志位、
	 * 某个事务是另一个事务的嵌套事务、但是这些事务又不在 Spring 的管理范围内、或者无法设置保存点
	 * 那么 spring 会通过设置回滚标志位的方式禁止提交、
	 * 外部事务判断当前事务流被设置了回滚标志、则由外部事务同一进行整体事务的回滚
	 * 在提交事务的过程、其实并不是直接提交、而是考虑诸多方面
	 *
	 * 对于嵌套事务、spring 正常的处理方式是、在嵌套事务开始之前设置保存点、一旦嵌套事务出现异常便根据保存点信息进行回滚
	 * 但是如果没有出现异常、嵌套事务并不会单独提交、而是由最外层的事务负责提交
	 * 所以如果当前存在保存点新、便不是最外层事务、不做保存操作、对于是否是新事物的判断也是基于此考虑
		...
		try {
			...
			try {
				...
				if (status.hasSavepoint()) {
					...
					// 如果有保存点、也就是当前事务为单独的线程、则会退到保存点（嵌套事务）
					...
					if (status.hasTransaction()) {
						if (status.isLocalRollbackOnly() || isGlobalRollbackOnParticipationFailure()) {
							...
							// 如果当前事务不是独立的事务、则只能标记状态、等到食物链执行完毕后同一回滚（JTA）
