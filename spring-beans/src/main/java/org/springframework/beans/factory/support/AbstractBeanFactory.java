...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	 ...
	 * Return an instance, which may be shared or independent, of the specified bean.
	 *
	 * @param name          the name of the bean to retrieve 检索的bean的name
	 * @param requiredType  the required type of the bean to retrieve 检索bean的 类型
	 * @param args          arguments to use when creating a bean instance using explicit arguments
	 *                      (only applied when creating a new instance as opposed to retrieving an existing one)
	 *                      仅仅是创建新实例的时候有用、而在检索的时候没有用
	 * @param typeCheckOnly whether the instance is obtained for a type check, not for actual use
	 *                      仅仅是为了类型检查而去获取这个 bean、并非为了实际去使用这个bean
	 * @return an instance of the bean
	 * @throws BeansException if the bean could not be created
	 */
				...
				if (mbd.isSingleton()) {
					...
					sharedInstance = getSingleton(beanName, () -> {
						try {
							...
							// 显式的从单例缓存中 因为它可能被放进去缓存了 由于 单例模式下解决循环依赖
	...
	/**
	 * 返回 bean name 、如果参数是一个 bean factory 的 name的话、那么就要去掉它的前缀字符串 &
	 * 然后我们去 AliasRegistry中找到最终的beanName
		...
		if (!this.alreadyCreated.contains(beanName)) {
			synchronized (this.mergedBeanDefinitions) {
				if (!this.alreadyCreated.contains(beanName)) {
					...
					// 设置 beanDefinition 的属性stale 为true (这个属性在get bean definition的时候会判断、如果是true的话就不返回 )
	...
	/**
	 * 如果有的话、就移除这个 beanName
	 * 但仅当 除类型检查以外 没有将其用于其他目的时
	...
	/**
	 * 1. 如果用户指明我要的是一个 factory bean 、那么你拿到也是 一个 factory bean 、那么就返回呗
	 * 2. 如果用户要的不是一个 factory bean、 并且你找到的也不是一个 factory bean、那么就直接返回啊
	 * 3. 但是、如果用户想要的是一个 普通的bean、也就是不是factory bean、那么你找到的是一个factory bean、那么你是不是要通过这个factory bean 得到一个普通的bean啊
	...
	protected Object getObjectForBeanInstance(
		...
		if (BeanFactoryUtils.isFactoryDereference(name)) {
			...
			// 如果这个是 NullBean 类型、这个就是说是ObjectFactory 返回来的
			if (beanInstance instanceof NullBean) {
				return beanInstance;
			}
			// 获取到的BeanInstance 不是一个 factory、但是你tm name 又带有这个 &  很迷惑啊兄弟
			if (!(beanInstance instanceof FactoryBean)) {
